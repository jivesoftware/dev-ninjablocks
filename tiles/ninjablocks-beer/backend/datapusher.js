...
function processTileInstance(instance) {
	...
	pushData(instance,-1,new Date(new Date().getTime()-300000)).then(
		function(success) {			
			jive.logger.debug('SUCCESS - Pushed Beer Temperature Color ['+temperature+']');
		},
		function(error) {
		}
	);
...
function pushData(temperature,timestamp) {
	var index = 2;
	var status = "Unknown";
	var message = "Waiting for Beer Temperature";
	
	if (temperature && temperature > 0) {
		//PRESERVE CELCIUS MEASUREMENT WITH 1 DECIMAL POINT
		message = ''+Math.round(temperature * 10)/10+' C / ';
		
		//CONVERT FROM CELCIUS TO FAHRENHEIT WITH 1 DECIMAL POINT
		temperature = Math.round((((temperature*9)/5)+32)*10)/10;
		
		message += temperature + ' F';
		
		message += ' @ ' + dateFormat("longTime");
		if (temperature >= 55) {
			index = 0;
			status = "Hot";
    ...
    var data = {
        	data : {
    	    "message" : message,
    	    "sections" : demo.beerTemperatures,
    	    "activeIndex": index,
    	    "status" : status
        	}
        };	
	
    jive.tiles.findByDefinitionName('ninjablocks-beer').then( function(instances) {
        if ( instances ) {
